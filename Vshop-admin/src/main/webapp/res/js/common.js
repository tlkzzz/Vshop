	    ...
	    if(result){
	       	...
	       	if(result.cart_goods_num >0){
    	        ...
    	        obj.find('.incart-goods-box').perfectScrollbar();
	         	html = "共<i>"+result.cart_goods_num+"</i>种课程&nbsp;&nbsp;总计金额：<em>&yen;"+result.cart_all_price+"</em>";
	      	...
	      	html="<div class='no-order'><span>您的购物车中暂无课程，赶快选择心爱的课程吧！</span></div>";
	      	obj.find('.incart-goods').html(html);
    ...
    $.getJSON(SITEURL+'/index.php?act=cart&op=del&cart_id='+cart_id+'&goods_id='+goods_id+'&callback=?', function(result){
        if(result.state){
            ...
            if(result.quantity == 0){
    	      	html="<div class='no-order'><span>您的课程表中暂无课程，赶快选择心爱的课程吧！</span></div>";
    	      	obj.find('.incart-goods').html(html);
    	      	obj.find('.total-price').html('');
		        obj.find('.addcart-goods-num').remove();
            }else{
                $('#cart_item_' + goods_id).remove();        //移除
	         	html="共<i>"+result.quantity+"</i>种课程&nbsp;&nbsp;总计金额：<em>&yen;"+result.amount+"</em>";
	         	obj.find('.total-price').html(html);
