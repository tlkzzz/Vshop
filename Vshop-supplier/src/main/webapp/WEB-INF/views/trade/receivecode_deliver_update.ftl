        							...
        							<#list order.orderGoodsList as orderGoods>
		                				<tr>
		            						...
		            						<#if orderGoods_index == 0>
			                    				<td class="bdl bdr order-info w380" rowspan="${orderGoods?size}">
			                    					<dl>
			              								<dt>运费：</dt>
			              								<dd>
			              									<#if shippingFee??>
	    					...
	    					<table class="ncu-table-style order" id="texpress2" >
	      						<tbody>
	          							...
	          							<td class="bdr tl w250">
					            				...
					            				<#if codeList??>
					            					<#list codeList as code>
					           						<div >${code.consumerCodeBunch}</div>     
					            					</#list>
					            				</#if>
	          							</td>
	        						</tr>
	        						<tr>
	          							...
	          							<td class="bdr tl w250">
	          								<input type="text" class="text" id="codesend"  placeholder="输入您要收回的码号" />
	          								<a nc_type='eb' nc_value="e1000" href="javascript:void(0);" class="ncu-btn6" onclick="receiveCode(this, '${order.orderId}');">确认</a>
	          							</td>
	        						</tr>
	        						<tr>
	          							<td colspan="2"></td>
...
<script type="text/javascript">
	function receiveCode(that, orderId){
		var $t = $(that).hide(0);
		$.post('${base}/transport/updateCode', {orderId: orderId, ConsumerCodeBunch: $('#codesend').val()}, function(result){
			if(result.success){
				location.href = '${base}/transport/receivecode?orderState=40';
			}else{
...
function confirmShipHSC(orderSn,id){
	...
	$.ajax({
		type: "post",
 		url: '${base}/transport/shipments_update',
		data: {orderSn:orderSn,deliverExplain:deliverExplain,daddressId:daddressId,shippingCode:shippingCode},
		dataType: "json",
		success:function(data) {
			if(data.success){
				layer.alert("发货成功!",{icon:1},function(){
