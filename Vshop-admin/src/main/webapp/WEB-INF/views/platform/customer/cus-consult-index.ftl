		    ...
		    <thead>
		      ...
		      <tr>
		        ...
		        <td colspan="20"><label for="all">全选</label>
		          <a href="javascript:void(0);" class="ncu-btn1" nc_type="batchbutton" uri="" name="id" confirm="您确定要删除吗?"><span>删除</span></a></td>
		      </tr>
		    </thead>
		    <tbody>
		    <#if datas?? >
		    	<#list datas as data>
			      ...
			      <tr>
			        <th colspan="20" class="tl"><input type="checkbox"  value="3" class="checkitem ml10 mr10" />
			          <span><a href="${base}/product/detail?id=${data.goodsIs}" target="_blank">${data.cgoodsName }</a></span><span class="ml20">咨询用户：</span>
			          ${data.cmemberName}<span class="ml20">咨询时间：<em class="goods-time">${data.consultAddtime?string('yyyy-MM-dd HH:mm:ss')}</em></span></th>
			      </tr>
			      <tr>
			        <td class="bdl w30"></td>
