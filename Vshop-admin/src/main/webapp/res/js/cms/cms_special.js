$(document).ready(function(){
    ...
    $("#btn_show_special_insert_goods").click(function(){
        html_form('dialog_special_insert_goods', '插入课程', $("#_dialog_special_insert_goods").html(), 640);
    });
    ...
    $("[nctype='btn_special_goods']").live('click', function(){
        ...
        $.getJSON("index.php?act=cms_special&op=goods_info_by_url", { url: url}, function(data){
            if(data.result == "true") {
                ...
                temp += '<dd class="taobao-item-delete" nctype="btn_special_goods_delete" title="删除添加的课程">&nbsp;</dd>';
                temp += '</dl></li>';
