package com.Vshop.supplier.utils.shiro;
...
//    private SellerService sellerService;
	...
	protected AuthorizationInfo doGetAuthorizationInfo(
		...
		/*
		 * Account account = accountDao.findByLoginName(loginName); List<Role>
		 * roleList = roleDao.getRoleByAcctId(account.getId()); Set<String>
		 * roleNameList = Sets.newHashSet(); for (Role role : roleList) {
		 * roleNameList.add(role.getRoleName()); } List<Permission>
		 * permissionList = permissionDao.getPermByRoleList(roleList);
		 * Set<String> permNameList = Sets.newHashSet(); for (Permission
		...
		/*
		 * Member member = memberService.findMemberByName(loginName); Store
		 * store = storeService.findByMemberId(member.getMemberId());
		 * if(store==null){ System.out.println("目前角色是－－－－－guest");
		 * info.addRole("guest"); }else{
		 */
		// System.out.println("目前角色是－－－－－seller");
		info.addRole("seller");
		// }
