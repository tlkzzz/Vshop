...
<style>
	...
	.box_arr .table_btn a.disable_spec { background: url(${base}/res/images/member/btn.gif) repeat 0 -1018px; float: right;}
    ...
    <form method="post" id="goods_form" action="#">
      ...
      <div class="goods-attribute" style="width: 98%;">
        ...
        <dl>
          <dt>商品分类：</dt>
          <dd id="gcategory">
                        <span class="f_l">${catename}</span> 
<!--                         <a class="edit_gcategory ncu-btn1 ml10" href="${base}/platform/sellIndex" > -->
                        <a class="edit_gcategory ncu-btn1 ml10" href="javascript:void(0)" >
                        <span>编辑</span>
                        </a>
        ...
        <!-- 
        <dl>
          <dt class="required"><span class="red">*</span>商家：</dt>
          <dd>
	          <select name="store_id" id="store">
		          <option value="" selected="selected">-请选择-</option>
		          <#if stores??>
		          	<#list stores as sts>
		          		<option value="${sts.storeId}" <#if sts.storeId == goods.storeId>selected="selected" </#if>>${sts.storeName}</option>
		          	</#list>
		          </#if>
	          </select>
          </dd>
        ...
        <dl>
          ...
          <dd nc_type="no_spec">
            ...
            <p class="hint">商品价格必须是0.01~1000000之间的数字</br>若启用了库存配置，请在下方商品库存区域进行管理</br>商品规格库存表中如有价格差异，商家价格显示为价格区间形式</p>
          </dd>
        </dl>
         ...
         <dl>
          ...
          <dd nc_type="no_spec">
            <p>
              <input name="goods_market_price" value="${goods.goodsMarketPrice}" type="text"  class="text" />
        ...
        <dl>
          ...
          <dd nc_type="no_spec">
            <p>
              <input name="goods_cost_price" value="${goods.goodsCostPrice}" type="text"  class="text" />
       ...
       <dl>
          ...
          <dd  nc_type="no_spec">
            ...
            <p class="hint">商家库存数量必须为1~1000000000之间的整数</br>若启用了库存配置，则系统自动计算商品的总数，此处无需商家填写</p>
          </dd>
       </dl>
        <dl>
        	...
        	<dd>
        		<div class="pic_list">
	                ...
	                <div class="content">
	                	...
	                	<div class="standard">
	                		<div>
	                			<ul style="height: 130px;overflow:auto;overflow-x: hidden;" id="photoView01">
	                				<#if imageList??>
		                				<#list imageList as imgSrc>
		                					<li style='height:120px;display:inline'><img class='img' style='width:100px;height:100px' src='${imgOssServer}${imgSrc}'/><a href='javascript:void(0)' imageSrc='${imgSrc}' name='deletePhoto'> 删除</a></li>
		                				</#list>
	                				</#if>
	                			</ul>
	                		</div>
       ...
       <!-- <dl>
            ...
            <p>
              <label style="padding-right:8px;">
	              <#if goods.isCode=0>
	                ...
	                是
	              <#else>
	              <input name="isCode" value="0" type="radio" />
	                是
	              </#if>
              </label>
              <label>
	              <#if goods.isCode==1>
	              	<input name="isCode" value="1" checked="checked" type="radio" />
	                否
	              <#else>
         ...
         <dl>
          ...
          <dd nc_type="no_spec">
            <p>
              <input name="goods_commission_rate" value="${goods.goodsCommissionRate}" type="text"  class="text" />
              <input name="goods_store_price_interval" value="" type="hidden"  />
              <!-- 价格区间 --> 
            </p>
          </dd>
        ...
        <dl>
     	...
     	<dl>
          <dt class="required"><span class="red">*</span>商品品牌：</dt>
          <dd>
        ...
        <dl>
          <dt class="required"><span class="red">*</span>供应商：</dt>
          <dd>
        ...
        <dl>
          ...
          <dd>
            <p style="float:left;  clear:both; margin-top:8px;">
              <input type="radio" value="1"   name="goods_transfee_charge" id="whops_seller">
              <label for="whops_seller">商家承担运费</label>
            </p>
            <p style="float:left;  clear:both; margin-top:8px;">
              <input type="radio" value="0"  name="goods_transfee_charge" id="whops_buyer">
              <label for="whops_buyer">用户承担运费</label>
            </p>
                 ...
                 <li>
                  ...
                  <div id="Postage" style="display:none">
                    <div class="freight-template">
                      <#if transport??>
                    		<select name="transportSelected" id="transportSelected">
					          <option value="${transport.id}" selected="selected">${transport.title}</option>
				        	</select>
                   	  </#if>
        ...
        <dl>
          ...
          <dd>
            ...
            <p class="mt5">
              ...
              <input onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});" class="txt" type="text" value="" id="time_to" name="endTime" style="width: 250px;height: 18px;"/>
            </p>
            <p class="mt5">
              <label>
...
var goodsStorePrice = "";  // 商品价格
var goodsMarketPrice = "";  // 商品价格
var goodsCostPrice = "";  // 商品价格
...
var isCode = "";  			//是否发码
var goodsCommissionRate = "";  //佣金比例
var transportId = "";
var goodsTransfeeCharge = "";//商品运费承担方式 默认 0为会员承担 1为商家承担
...
var specName = '${goods.specName}';     // 是否有规格
var imgBasePath = '${imgOssServer}'; 
		...
		var id = $(this).val();
	  	$.ajax({
			 ...
			 success:function(data) {
			 	var $li ='<select name="supplier_id" id="supplier">';
...
//	alert('${goods.goodsTransfeeCharge}');
//	if('${goods.goodsTransfeeCharge}' == "0"){
//		
//		$("input[type=radio][name=goods_transfee_charge][value=0]").attr("checked",'checked');
//		$("#whops_buyer_box").css("display","");
//		if('${goods.transportId}' != "0"){
//			$("[name=isApplyPostage][value=1]").click();
//		}
//	}else{
//		
//		$("input[type=radio][name=goods_transfee_charge][value=1]").attr("checked",'checked');
//		$("#whops_buyer_box").css("display","none");
//	}
};
