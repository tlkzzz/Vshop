...
$(document).ready(function(){
    ...
    $("[nctype='btn_module_goods_edit']").click(function(){
        ...
        goods_list.find("dl").append('<dd class="taobao-item-delete" nctype="btn_goods_delete" title="删除添加的课程">&nbsp;</dd>');
        limit_count = parseInt($(this).attr("limit_count"), 10);
        if(!limit_count) {
            limit_count = 0;
        }
        $('#dialog_module_goods_edit').nc_show_dialog({width: 640, title: '编辑课程'});
            ...
            if(link != '') {
                ...
                $.getJSON('index.php?act=cms_base&op=goods_info_by_url', { url: url}, function(data){
                    if(data.result == "true") {
                        ...
                        temp += '<dd class="taobao-item-delete" nctype="btn_goods_delete" title="删除添加的课程">&nbsp;</dd>';
                        temp += '</dl></li>';
