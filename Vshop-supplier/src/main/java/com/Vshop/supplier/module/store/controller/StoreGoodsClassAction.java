 ...
 * 商户自分类
 * 
 * 项目名称：Vshop-supplier 类名称：StoreGoodsClassAction 类描述： 创建人：weiyue 创建时间：2014年12月1日
 * 下午8:34:56 修改人：weiyue 修改时间：2014年11月26日 下午8:34:56 修改备注：
 * 
 * @version
 * 
 */
...
public class StoreGoodsClassAction {
	...
	public ModelAndView index() {
		ModelAndView model = new ModelAndView("/store/store-class");
		StoreGoodsClassVo storeGoodsClassVo = new StoreGoodsClassVo();
		storeGoodsClassVo.setStoreId(CacheUtils.getCacheUser().getStore().getStoreId());
		
		StoreGoodsClass storeGoodsClass1 = new StoreGoodsClass();
		storeGoodsClass1.setStoreId(CacheUtils.getCacheUser().getStore().getStoreId());
		...
		model.addObject("storeGoodsClassListT", storeGoodsClassListT);
		return model;
	}
	...
	public ModelAndView findAll() {
		ModelAndView model = new ModelAndView("/store/store-class-list");
		StoreGoodsClassVo storeGoodsClassVo = new StoreGoodsClassVo();
		storeGoodsClassVo.setStoreId(CacheUtils.getCacheUser().getStore().getStoreId());
	...
	public ModelAndView editIndex(
        ...
        List<StoreGoodsClass> list = storeGoodsClassService.findParentList(CacheUtils.getCacheUser().getStore().getStoreId());
        StoreGoodsClass storeGoodsClass=storeGoodsClassService.selectByPrimaryKey(stcId);
		ModelAndView model = new ModelAndView("/store/store-class-edit");
		model.addObject("datas",list);
	...
	public ModelAndView addIndex(
	    ...
	    Integer scid=CacheUtils.getCacheUser().getStore().getStoreId();
        List<StoreGoodsClass> list = storeGoodsClassService.findParentList(CacheUtils.getCacheUser().getStore().getStoreId());
		ModelAndView model = new ModelAndView("/store/store-class-add");
	...
	Map<String, String> saveOrUpdate(
        ...
        storeGoodsClass.setStoreId(CacheUtils.getCacheUser().getStore().getStoreId());
