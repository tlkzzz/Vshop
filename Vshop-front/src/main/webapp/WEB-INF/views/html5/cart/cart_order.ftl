...
jQuery(document).ready(function() {
	...
	//提交订单事件
		...
		if(storeId==null||storeId==""){
			layer.alert('订单已超时,请您重新提交订单!',{icon:2});
			return false;
		}
		if (ifsubmit == true){
			...
			$.ajax({
		        ...
		        success : function(data){
		            ...
		            if(data.success){
		            	...
		            	else if(result.specnotfund.length!=0){ //判断商品是否存在
		            		//新建一个字符串,存储价格变动的商品名称
		            		var str = "";
...
function getTotalPrice(){
        ...
        success:function(data){
        	if(data.success){
        		$("#order_amount").html("应付金额:  &yen;"+toDecimal2(data.result.totalPrice));
...
function updateInvoice() {
    var addressId = $('input[name="address_options"]:checked').val();
    if(addressId==null) {
    	addressId = "";
    }
   	window.location.href='${base}/m/invoice/update?cartIds=${cartIds}&addressId='+addressId;
}
...
<img src="${base}/res/html5/images/back.png" width="25" height="25" /></a>
填写订单<a class="menu home" href="${base}/m/index/index">
...
<input type="hidden" name="cartIds" value="${cartIds}" id="cartIds" /> 
  <div class="fill_order" id="form_div">    
     ...
     <div class="pay_infro">
	     <ul>
	       ...
	       <li> 
	     	...
	     	<li>
	     		<!-- <a href="javascript:void(0);" onClick="choose_fp();"> -->
		     		<span class="fl">发票信息
		     			<span class="gray" id="fp_html">
		     				<#if invoiceStr!=null>
		     					${invoiceStr}
		     				<#else>
		     					不开发票 
		     		...
		     		<span class="fr">
		     			...
		     			<a href="javascript:void(0);" style="color: #666;" onClick="updateInvoice();">
		     			<i><img src="${base}/res/html5/images/mod.png" style="vertical-align:middle;" width="20px" height="20px" /></i>
		     			修改</a>
		     		</span>
	     		<!-- </a> -->
	     	</li>
    ...
    <div class="pay_shop">
	      ...
	      <input type="hidden" name="storeId"  id="storeId" value="${cartVo.storeId}" />
		      <h3>
			      <span class="fl">
    ...
    <div class="car_pay order_tt">
      <div class="total">
        <p class="price" id="order_amount" style="text-align:center; font-family:宋体;">
        	<#--
        	<em id="order_amount">
        		<script type="text/javascript">
