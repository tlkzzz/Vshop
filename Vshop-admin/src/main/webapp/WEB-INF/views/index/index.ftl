		 ...
		 success:function(data) {
			 ...
			 if(menus.length>0){
		        ...
		        for (var i = 0; i < menus.length; i++) {
		            var menu = menus[i];
		            menuHtml+='<li><a class="link" id="'+menu.id+'" href="javascript:;" onclick="openItem(\''+menu.id+'\');"><span>'+menu.name+'</span></a></li>';
		            var subMenus = menu.subMenu;
		            subMenuHtml+='<ul id="sort_'+menu.id+'"><li><dl><dd><ol>';
		            for (var j = 0; j < subMenus.length; j++) {
		                var sub = subMenus[j];
		                subMenuHtml+='<li><a href="javascript:void(0);" url="'+APP_BASE+sub.url+'" name="'+sub.id+'" id="'+sub.id+'" onclick="openItem(\''+menu.id+'\',\''+sub.id+'\');">'+sub.name+'</a></li>';
		            }
...
<table style="width: 100%;" id="frametable" height="100%" width="100%" cellpadding="0" cellspacing="0">
    ...
    <tr>
        <td colspan="2" height="90" class="mainhd"><div class="layout-header"> <!-- Title/Logo - can use text instead of image -->
            <div id="title"><a href="${base}"><img src="${imgServer}${siteSet.siteLogo2}" alt="" style="width:162px; height:40px;"/></a></div>
