package com.Vshop.front.wechat.service;
...
import com.Vshop.front.pay.mobile.wxpay.util.XMLUtil;
import com.Vshop.front.wechat.util.MD5Util;
import com.Vshop.front.wechat.util.OrderUtil;
import com.Vshop.front.wechat.util.WXMLUtil;
...
public class WXPrepay {
	private final static Logger logger = Logger.getLogger(WXPrepay.class);
	...
	public Map<String, String> reqOrderquery() {
		...
		try {
			...
			if (httpEntity != null) {
				...
				// 过滤
				result = result.replaceAll("<![CDATA[|]]>", "");
				map = WXMLUtil.doXMLParse(result);
			}
