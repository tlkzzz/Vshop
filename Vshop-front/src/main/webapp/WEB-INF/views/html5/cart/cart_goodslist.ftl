...
<script>
  ...
  function goods_count_adjust(gc_id,count,gift_id){
	  ...
	  if(gcs==""){
	     gcs="-1";//为了和顶部的购物车计算总价区分开，如果gcs为空则会计算购物车所有商品总价，为-1则不会计算任何商品
	  }
			...
			<#assign cartMap = cartTag() />
	       	<#if cartMap.get("cartVoList")??>
	    		<#list cartMap.get("cartVoList") as cartVo>
		            <p style="line-height:24px; font-size:14px; margin-top:3px; margin-left:5px;">商家：<a href="#">${cartVo.storeName}</a></p>                    	
		        	<#if cartVo.list??>
