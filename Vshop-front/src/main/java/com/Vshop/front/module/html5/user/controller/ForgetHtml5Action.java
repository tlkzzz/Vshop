 ...
 */
@Controller
@RequestMapping("/m/forget")
@Slf4j
public class ForgetHtml5Action {
...
//	private MessageService messageService;
	...
	public ModelAndView index(HttpServletRequest request){
		try {
			...
			ModelAndView model = new ModelAndView("/html5/user/forget/forget");
	...
	public ModelAndView activate(@RequestParam(value = "username") String username,
		...
		try {
			ModelAndView model = new ModelAndView("/html5/user/forget/forget-reset");
			
			Member member = memberService.findMemberByName(username);
			
			if(!code.equals(member.getMemberPasswd())){
				model = new ModelAndView("/html5/user/forget/forget-error");
			}
	...
	public @ResponseBody
	Map<String, String> updatePass(@RequestParam(value = "password") String password,
			@RequestParam(value = "new_password") String new_password,
			Model model) throws Exception {
		Map<String, String> map = Maps.newHashMap();
        int result = memberService.updatePass(password, new_password, CacheUtils.getCacheUser().getMember().getMemberId());
		if(result == 1){
			map.put("success", "true");
	...
	public ModelAndView success() {
		try {
			ModelAndView model = new ModelAndView("/html5/user/forget/forget-success");
			return model;
		} catch (Exception e) {
			log.error("找回密码首页加载失败！",e);
			throw new RuntimeException("导航失败!");
	...
	public @ResponseBody Map<String, String> sendValid(@RequestParam(value = "mobile") String mobile,
		...
		try {
			...
			//log.debug("随机验证码=="+ valid);
			//String returnMsg = messageService.send(mobile, "短信验证码："+valid+"。");
	...
	public String checkValidCode(@RequestParam(value="validCode") String validCode, 
		...
		if(validCode==""||mobile==""||code==null||recordTime==null){
			...
			return "/html5/user/forget/forget";
		}
		...
		return "forward:/m/forget/toRest?mobilep="+mobile+"&vCode="+validCode+"&flags="+flags;
	}
	...
	public ModelAndView toRest(@RequestParam(value = "mobilep") String mobilep,
		...
		try {
			...
			if(member==null){
				model =new ModelAndView("/html5/user/forget/forget");
