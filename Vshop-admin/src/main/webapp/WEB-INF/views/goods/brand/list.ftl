...
<script type="text/javascript">
    ...
    function delBrand(){
    	var brandIds = (function(){var _ids = []; $("input[name='ids']:checked").each(function(){_ids.push($(this).val())});return _ids})();
    ...
    function delValide(brandIds){
    	$.post("${base}/goods/brand/delValide", {ids: JSON.stringify(brandIds)}, function(result){
    		if(result.success){
    			$('#form_list').submit();
    		}else{
    			var data = result.data, goods = [], gn = [], supplier = [], sn = [], info = [];
    			_.each(data, function(e){e.goodsNum > 0 && goods.push(_.format(".{0}", e.brandId)), e.supplierNum > 0 && supplier.push(_.format(".{0}", e.brandId))});
    			!_.isEmpty(goods) && $(goods.join(',')).each(function(){gn.push($(this).text())});
    			!_.isEmpty(supplier) && $(supplier.join(',')).each(function(){sn.push($(this).text())});
    			!_.isEmpty(gn) && info.push(_.format("名为{0}的品牌有产品关联\n", JSON.stringify(gn)));
    			!_.isEmpty(sn) && info.push(_.format("名为{0}的品牌有机构关联\n", JSON.stringify(sn)));
    			info.push(result.msg);
    			alert(info);
    		}
    	}, "JSON");
    }
        ...
        <table class="table tb-type2">
            ...
            <#list pager.result as brand>
	            <tr class="hover edit">
	                ...
	                <td class="name">
	                    <span  nc_type="inline_edit" fieldname="brandName" modUrl="${base}/goods/brand/modifyName" fieldid="${brand.brandId}" required="1" title="可编辑" class="${brand.brandId}">${brand.brandName}</span>
	                </td>
	                <td class="class">
	                    ${brand.brandClass}
	                </td>
