...
<div class="page">
    ...
    <form method="post" name="formSearch" id="formSearch" action="${base}/goods/supplier/list">
        ...
        <table class="tb-type1 noborder search">
            <tbody>
            <tr>
                <th><label for="search_supplier_name">供应商名称</label></th>
                <td><input class="txt" name="name" id="search_supplier_name" value="${supplier.name}" type="text" style="width:260px;"></td>
                <th><label for="search_supplier_deleted">状态</label></th>
                <td>
                	<select name="deleted" class="class-select" style="width:90px;">
                        <option value="">请选择...</option>
                        <option <#if supplier.deleted == 0>selected="selected"</#if> value="0">正常</option>
                        <option <#if supplier.deleted == 1>selected="selected"</#if> value="1">删除</option>
                    </select>
                </td>
                <td><a href="javascript:void(0);" id="ncsubmit" class="btn-search " title="检索">&nbsp;</a>
                    <#if supplier.name != '' || supplier.deleted != ''>
        ...
        <table class="table tb-type2">
            ...
            <#list pager.result as supplier>
	            <tr class="hover edit">
				    ...
				    <td class="align-center">
	                     <#if supplier.deleted == 1>删除<#else>正常</#if>
				    </td>
	                <td class="align-center">
