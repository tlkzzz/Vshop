...
<form id="store_class_form" name="floorForm" method="post" action="${base}/website/index/save">
    <table class="table tb-type2">
        ...
        <tr class="noborder">
         ...
         <tr class="noborder">
            <td colspan="2" class="required"><label>楼层右边广告图片：</label></td>
        </tr>
        <tr class="noborder">
        	<input type="hidden" name="ggImg" id="ggImg"/>
        	<input type="hidden" name="ggUrl" id="ggUrl"/>
            <td class="vatop rowform">
                <img id="floorggImg" width="210px" src="${base}/res/images/noslide.png"/>
                <input type="button" value="编辑" onclick="show_dialog('upload_floorggBanner','楼层右边广告图片');"/>
            </td>
            <td class="vatop tips">请上传210×40像素GIF\JPG\PNG格式图片</td>
        </tr>
...
<#--右侧广告位 开始-->
<div id="upload_floorggBanner_dialog" style="display:none;">
    <table class="table tb-type2">
        <tbody>
        <tr>
            <td><ul>
                <li>请按照操作注释要求，上传设置楼层右边广告上传。</li>
            </ul></td>
        </tr>
        </tbody>
    </table>
    <table id="upload_tit_" class="table tb-type2">
        <tbody>
        <tr>
            <td colspan="2" class="required">右边广告上传：</td>
        </tr>
        <tr class="noborder">
            <td class="vatop rowform"><span class="type-file-box">
            <input type='text' name='textfield' id='textfield3' class='type-file-text' />
            <input type='button' name='button' id='button1' value='' class='type-file-button' />
            <input name="pic" id="pic2" type="file" class="type-file-file" size="30">
            </span></td>
            <td class="vatop tips">建议上传210×40像素GIF\JPG\PNG格式图片。</td>
        </tr>
        </tbody>
    ...
    <table id="upload_tit_type_txt" class="table tb-type2">
        <tbody>
        <tr>
            <td colspan="2" class="required">右边广告链接</td>
        </tr>
        <tr class="noborder">
            <td class="vatop rowform">
                <input class="txt" type="text" id="bggUrl">
            </td>
            <td class="vatop tips"></td>
        </tr>
        </tbody>
    </table>
    <a href="JavaScript:void(0);" onclick="bannerImgCommitst()" class="btn"><span>确定</span></a>
    <script type="text/javascript">
        $("#pic2").change(function(){
            $("#textfield3").val($(this).val());
        });
        function bannerImgCommitst(){
            $.ajaxFileUpload({
                //处理文件上传操作的服务器端地址(可以传参数,已亲测可用)
                url:'${base}/website/index/upload',
                secureuri:false,                       //是否启用安全提交,默认为false
                fileElementId:'pic2',           //文件选择框的id属性
                dataType:'json',                       //服务器返回的格式,可以是json或xml等
                success:function(data, status){        //服务器响应成功时的处理函数
                    if(data.success){     //0表示上传成功(后跟上传后的文件路径),1表示失败(后跟失败描述)
                        $("#floorggImg").attr("src", '${imgServer}'+data.result);
                        $("#ggImg").val(data.result);//设置分类图片
		            	$("#ggUrl").val($("#bggUrl").val());
                        
                        DialogManager.close("upload_floorggBanner");
                    }
                },
                error:function(data, status, e){ //服务器响应失败时的处理函数
                    alert('图片上传失败，请重试！！');
        ...
        function get_recommend_goods() {//查询商品
            ...
            if (gc_id>0 || goods_name!='') {
                ...
                $.ajax({
                    ...
                    data:{div:div,gcId:gc_id,goodsName:goods_name,goodsShow:"1",goodsState:"0"},
