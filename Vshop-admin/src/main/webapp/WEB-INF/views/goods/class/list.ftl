...
<script type="text/javascript"
	src="${base}/res/js/jquery.goods_class.js"></script>
<script type="text/javascript">
	var vadate = false;
	function delClass() {
	...
	<table class="table tb-type2" id="prompt">
		<tbody>
			...
			<tr style="background: none repeat scroll 0% 0% rgb(255, 255, 255);"
				class="odd">
				<td><ul>
						<li>当院长添加商品时可选择商品分类，用户可根据分类查询商品列表</li>
	...
	<form method="post" id='form_list' action="${base}/goods/class/delete">
		<table class="table tb-type2" id="acct_grid">
			...
			<tbody>
				...
				<tr class="hover edit">
					<td class="w48">
						<input type="checkbox" name="ids" value="${class.gcId}" class="checkitem"> 
						<#if class.hasChild != 0> 
					...
					<td class="w48 sort"><span title="可编辑" datatype="number"
						modUrl="${base}/goods/class/modifySort" fieldid="${class.gcId}"
						fieldname="gcSort" nc_type="inline_edit" class="editable ">
							${class.gcSort}</span>
					</td>
					<td class="w50pre name">
					<span title="可编辑" required="1"
						fieldid="${class.gcId}" modUrl="${base}/goods/class/modifyName"
						fieldname="gcName" nc_type="inline_edit" class="editable ">${class.gcName}</span>
					...
					<td class="w84"><a
						href="${base}/goods/class/forward?gcId=${class.gcId}&gcParentId=${class.gcId}">编辑</a>
			...
			<tfoot>
				<tr class="tfoot">
					...
					<td id="batchAction" colspan="15"><span class="all_checkbox">
						...
						href="JavaScript:void(0);" class="btn" onclick="delClass()"> <span>删除</span>
					</a></td>
				</tr>
			</tfoot>
		</table>
