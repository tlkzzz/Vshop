    ...
    <div class="fixed-bar">
        <div class="item-title">
            ...
            <ul class="tab-base">
                ...
                <li><a href="${base}/platform/storeList" ><span>下架商品</span></a></li>
        ...
        <table class="tb-type1 noborder search">
            ...
            <tr>
				...
				<select name="area" id="area">
                    ...
                    <input id="memberProvinceid" type="hidden" name="memberProvinceid" value="${memberProvinceid}">
                    <input id="city_id" type="hidden" name="city_id" value="${city_id}">
                    <input id="area_id" class="area_ids" type="hidden" name="area_id" value="${area_id}">
                    <input id="area_info" class="area_names" type="hidden" name="area_info" value="${area_info}">
                </td>
            ...
            <#if pager.result??>
	            ...
	            <tr class="hover edit">
	                ...
	                <td class="align-center">
	                	...
	                	<p><a href="javascript:void(0);" onClick="updteMoney('${goods.goodsId}')">审核</a></p>
      ...
      */
      ...
      function combinationDialog(goodsId,storeId) {
       ...
       /**
  		修改价格
  */
  function updteMoney(goodsId){
  		layer.open({
  			type : 2,
  			move : false,
  			shade : false,
  			title : '审核',
  			content : ['${base}/platform/toUpdateMoney?goodsId='+goodsId,'no'],
  			area : ['500px', '500px'],
  			btns : 2,
  			btn : ['确定','取消'],
  			yes : function(index,goodsId){
  				var goodsState = layer.getChildFrame(':checked').val();
  				var err = layer.getChildFrame('[id=errmsg]').html();
  				
  				var goodsMarketPrice = layer.getChildFrame('[name=goods_market_price]').val();
  				var goodsStorePrice = layer.getChildFrame('[name=goods_store_price]').val();
  				var goodsCommissionRate = layer.getChildFrame('[name=goods_commission_rate]').val();
  				var goodsId = layer.getChildFrame('[id=goodsId]').val();
  				var goodsCloseInfo = layer.getChildFrame('[name=goodsCloseInfo]').val();
  				var isSuccess = 0;
	  			if(checkMoney(isSuccess)==1){
		  			if(''==err){
			  			$.ajax({
			  					type : "POST",
			  					url : "${base}/platform/updateMoney",
			  					data : {
			  					goodsId:goodsId,
  ...
  function checkMoney(isSuccess){
  		var goodsState = layer.getChildFrame(':checked').val();
		var goodsMarketPrice = layer.getChildFrame('[name=goods_market_price]').val();
		var goodsStorePrice = layer.getChildFrame('[name=goods_store_price]').val();
		var goodsCommissionRate = layer.getChildFrame('[name=goods_commission_rate]').val();
		var goodsCostPriceInventory = layer.getChildFrame('[name=goods_cost_price_inventory]').val();
		var errmsg = layer.getChildFrame('[id=errmsg]');
		var isNum = /^\d+(\.\d+)?$/;
		if(goodsState=="0"){
		
				if(!isNum.test(goodsMarketPrice)){
					layer.alert('请输入正确的市场价格');
					return isSuccess = 0;
		        }else if(parseFloat(goodsMarketPrice)<0.01 || parseFloat(goodsMarketPrice)> 1000000 ){
					layer.alert('市场价格必须是0.01~1000000之间的数字');
					return isSuccess = 0;
				}else if(parseFloat(goodsMarketPrice)<parseFloat(goodsCostPriceInventory)){
					layer.alert('市场价格不能小于成本价格');
					return;
				}else{
					errmsg.html('');
					isSuccess = 1;
				}
		        if(!isNum.test(goodsStorePrice)){
					layer.alert('请输入正确的成本价格');
					return isSuccess = 0;
		        }else if(parseFloat(goodsStorePrice)<0.01 || parseFloat(goodsStorePrice)> 1000000 ){
					layer.alert('市场价格必须是0.01~1000000之间的数字');
					return isSuccess = 0;
				}else if(parseFloat(goodsStorePrice)<parseFloat(goodsCostPriceInventory)){
					layer.alert('成本价格不能大于商品价格');
					return;
				}else{
					errmsg.html('');
					isSuccess = 1;
		...
		        if(!isNum.test(goodsCommissionRate)){
					layer.alert('请输入正确的佣金比例');
					return isSuccess = 0;
		        }else if(parseFloat(goodsCommissionRate)<0.01 || parseFloat(goodsCommissionRate)> 1){
					layer.alert('佣金必须是0.01~1之间的数字');
					return isSuccess = 0;
				}else{
					errmsg.html('');
					isSuccess = 1;
